...
export class LivingRoomDoorScene extends BaseScene {
  constructor() {
    super(SceneKeys.LIVING_ROOM_DOOR);
  }
  ...
  protected initializeScene(): void {
    ...
    // 添加房间标题
    TextRenderer.createCenteredText(this, 640, 50, '客厅门口', {
      fontSize: '32px',
      color: '#000000',
      fontStyle: 'bold'
    });
    ...
    // 获取房间数据
    const roomData = this.sceneManager?.getRoomData(RoomKeys.LIVING_ROOM_DOOR);
    if (!roomData) return;
    ...
    if (this.audioManager) {
      this.audioManager.playRoomMusic(RoomKeys.LIVING_ROOM_DOOR);
    }

    // 记录房间访问
    if (this.gameManager) {
      this.gameManager.visitRoom(RoomKeys.LIVING_ROOM_DOOR);
    }
  ...
  private setupKeyboardShortcuts(): void {
    ...
    this.input.keyboard?.on('keydown-ONE', () => {
      this.executeAction('look_outside');
    });
  }
} 